...
@Component({
  ...
  `,
  styles: [`
    .base-table{display:grid;gap:14px}
    .table-header{display:flex;align-items:flex-start;justify-content:space-between;gap:14px}
    h2{margin:0;color:var(--text);font-size:1.25rem}p{margin:6px 0 0;color:var(--muted)}
    .table-tools{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap}
    .search-box{display:flex;align-items:center;gap:8px;min-width:min(280px,100%);padding:0 12px;border:1px solid var(--line);border-radius:18px;background:var(--surface-muted);color:var(--muted)}
    .search-box input{border:0;background:transparent;padding:11px 0;box-shadow:none}
    .table-shell{position:relative;overflow:hidden;border:1px solid var(--line);border-radius:24px;background:color-mix(in srgb,var(--surface) 88%,transparent)}
    table{width:100%;border-collapse:collapse}
    th,td{padding:14px 16px;text-align:start;border-bottom:1px solid var(--line);vertical-align:middle}
    th{color:var(--muted);font-size:.84rem;font-weight:950;background:color-mix(in srgb,var(--surface-muted) 80%,transparent)}
    td{color:var(--text);font-weight:800}tbody tr:last-child td{border-bottom:0}
    .row-actions{display:flex;gap:8px;flex-wrap:wrap}
    .table-action{display:inline-flex;align-items:center;justify-content:center;gap:7px;min-height:36px;border:1px solid var(--line);border-radius:999px;padding:8px 12px;background:var(--surface-muted);color:var(--text);font:inherit;font-size:.82rem;font-weight:950;cursor:pointer}
    .table-action.primary,.table-action.add{background:linear-gradient(135deg,var(--brand),var(--brand-2));color:#1b1712;border-color:transparent}
    .table-action.danger{background:color-mix(in srgb,var(--danger) 18%,var(--surface-muted));color:#fecaca}
    ...
    @media (max-width:760px){
      ...
      .table-shell{overflow:visible;border:0;background:transparent}table,thead,tbody,tr,th,td{display:block}thead{display:none}
      tbody{display:grid;gap:10px}tr{padding:12px;border:1px solid var(--line);border-radius:22px;background:color-mix(in srgb,var(--surface) 90%,transparent);box-shadow:0 12px 34px rgba(0,0,0,.16)}
      td{display:grid;grid-template-columns:minmax(92px,.7fr) 1fr;gap:10px;padding:8px 0;border-bottom:1px dashed var(--line);font-size:.9rem}td:last-child{border-bottom:0}
      td::before{content:attr(data-label);color:var(--muted);font-weight:950}.row-actions{justify-content:flex-start}.table-action{min-height:34px;padding:7px 10px}
    }
  `]
})
