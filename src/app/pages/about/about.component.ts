...
@Component({
  ...
  `,
  styles: [`
    .about-hero{display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,.55fr);gap:28px;align-items:center;padding-top:130px}.about-hero h1{font-size:clamp(1.9rem,4vw,3.35rem);margin:0 0 16px}.doctor-card{padding:30px;border:1px solid var(--line);border-radius:42px;background:radial-gradient(circle at 10% 0,color-mix(in srgb,var(--gold) 24%,transparent),transparent 46%),color-mix(in srgb,var(--surface) 82%,transparent);box-shadow:var(--shadow);animation:doctorFloat 5s ease-in-out infinite alternate}.doctor-card .icon-bubble{width:70px;height:70px;font-size:2rem}.story-panel{display:grid;grid-template-columns:minmax(0,.75fr) minmax(0,1fr);gap:26px;align-items:center;padding:34px;border:1px solid var(--line);border-radius:42px;background:color-mix(in srgb,var(--surface) 82%,transparent);box-shadow:var(--shadow)}@keyframes doctorFloat{to{transform:translateY(-12px)}}@media(max-width:820px){.about-hero,.story-panel{grid-template-columns:1fr;padding-top:112px}.story-panel{padding:24px}}
  `]
})
