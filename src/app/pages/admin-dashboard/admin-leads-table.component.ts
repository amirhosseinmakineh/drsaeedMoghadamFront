...
export class AdminLeadsTableComponent implements OnChanges, OnInit {
  ...
  private leadState(row: LeadAssignmentItem): number | null {
    return this.numberOrNull(row.leadAssignmentState ?? row.LeadAssignmentState ?? row.state ?? row.State ?? row.status ?? row.Status ?? null);
  }

  private leadType(row: LeadAssignmentItem): number | null {
    return this.numberOrNull(row.leadAssignmentType ?? row.LeadAssignmentType ?? row.assignmentType ?? row.AssignmentType ?? row.type ?? row.Type ?? null);
  }

  private leadId(row: LeadAssignmentItem): number | null {
    return this.numberOrNull(row.id ?? row.Id ?? row.leadAssignmentId ?? row.LeadAssignmentId ?? null);
  }
