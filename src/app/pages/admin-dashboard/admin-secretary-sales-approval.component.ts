...
export class AdminSecretarySalesApprovalComponent implements OnInit {
  ...
  review(): void {
    ...
    request.pipe(finalize(() => { this.reviewing = false; this.cdr.markForCheck(); }), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (result) => { if (!result.isSuccess) { this.toast.error(result.message); return; } this.toast.success(result.message); this.target = null; this.load(); }, error: (error) => this.toast.error(error?.error?.message || "بررسی فروش انجام نشد.") });
  }
