...
import { AdminDashboardService, ReservationAttendanceReview } from '../../core/admin/admin-dashboard.service';
    ...
    <section class="admin-panel">
      <header class="panel-heading">
        <div>
          ...
          <p>ادعای حضور/عدم حضور ثبت‌شده توسط مشاور را بررسی و تایید یا رد کنید.</p>
        </div>
        ...
        <div class="review-table">
          @for (item of items; track reservationId(item)) {
            <article>
              <header>
                <div>
                  <strong>{{ value(item.patientName, item.PatientName, 'بیمار بدون نام') }}</strong>
                  <small>{{ value(item.patientPhoneNumber, item.PatientPhoneNumber, '-') }} - {{ value(item.patientCity, item.PatientCity, 'شهر ثبت نشده') }}</small>
                </div>
                <b [class]="statusBadge(status(item))">{{ statusLabel(status(item)) }}</b>
              </header>
              <dl>
                <div><dt>مشاور</dt><dd>{{ value(item.consultantName, item.ConsultantName, '-') }}</dd></div>
                <div><dt>زمان رزرو</dt><dd>{{ formatDate(value(item.reservationAt, item.ReservationAt, '')) }}</dd></div>
                <div><dt>احتمال حضور</dt><dd>{{ probability(item) }}٪</dd></div>
                <div><dt>پیش‌بینی مشاور</dt><dd>{{ value(item.attendancePrediction, item.AttendancePrediction, '-') }}</dd></div>
                <div><dt>یادداشت مشاور</dt><dd>{{ value(item.consultantAttendanceNote, item.ConsultantAttendanceNote, '-') }}</dd></div>
              </dl>
              <label>یادداشت منشی<textarea [(ngModel)]="notes[reservationId(item) || 0]" [name]="'secretaryNote' + reservationId(item)" rows="2"></textarea></label>
              <div class="dialog-actions">
                <button class="primary-action compact" type="button" [disabled]="savingId === reservationId(item)" (click)="review(item, true)">تایید ادعای مشاور</button>
                <button class="secondary-action compact danger" type="button" [disabled]="savingId === reservationId(item)" (click)="review(item, false)">رد ادعای مشاور</button>
              </div>
            </article>
          }
        </div>
...
export class SecretaryReservationAttendanceReviewsComponent implements OnInit {
  ...
  constructor(private adminApi: AdminDashboardService, private auth: AuthService) {}
  ...
  load(): void {
    this.loading = true;
    this.feedback = '';
    this.adminApi.getReservationAttendanceReviews().pipe(finalize(() => this.loading = false)).subscribe({
      next: response => this.items = response.items ?? [],
      error: error => this.showFeedback(error instanceof Error && error.message ? error.message : 'دریافت صف بررسی انجام نشد', 'error')
    });
  }
  ...
  review(item: ReservationAttendanceReview, approved: boolean): void {
    ...
    if (!reservationId || !secretaryUserId) {
      this.showFeedback('شناسه رزرو یا شناسه کاربر منشی در دسترس نیست', 'error');
      return;
    }

    this.savingId = reservationId;
    this.adminApi.reviewAttendance({
      reservationId,
      secretaryUserId,
      approved,
      note: (this.notes[reservationId] || '').trim() || null
    }).pipe(finalize(() => this.savingId = null)).subscribe({
      next: response => { this.showFeedback(response.message || 'بررسی حضور ثبت شد', 'success'); this.load(); },
      error: error => this.showFeedback(error instanceof Error && error.message ? error.message : 'ثبت بررسی انجام نشد', 'error')
    });
  }

  reservationId(item: ReservationAttendanceReview): number | null {
    const value = item.id ?? item.Id ?? item.reservationId ?? item.ReservationId;
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : null;
  }

  status(item: ReservationAttendanceReview): number | null {
    const numeric = Number(item.attendanceConfirmationStatus ?? item.AttendanceConfirmationStatus);
    return Number.isFinite(numeric) ? numeric : null;
  }

  probability(item: ReservationAttendanceReview): number | string {
    return item.attendanceProbabilityPercent ?? item.AttendanceProbabilityPercent ?? '-';
  }
  ...
  statusBadge(value: number | null): string { return value === 2 ? 'success' : value === 3 ? 'warn' : 'danger'; }
  ...
  formatDate(value: string): string {
    ...
    return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat('fa-IR', { dateStyle: 'medium', timeStyle: 'short' }).format(date) : value;
  }
