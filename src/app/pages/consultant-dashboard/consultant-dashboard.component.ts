        ...
        <form class="dialog-form patient-profile-form" (ngSubmit)="submitPatientProfile()">
          <section class="form-section">
            ...
            <div class="two-col">
              ...
              <label>
                شماره اضطراری
                <input [(ngModel)]="patientProfileForm.emergencyPhoneNumber" name="patientEmergencyPhone" inputmode="tel" placeholder="اختیاری" />
              </label>
            </div>

            <label>
              آدرس بیمار
              <textarea [(ngModel)]="patientProfileForm.address" name="patientAddress" rows="3"></textarea>
            </label>

            <div class="two-col">
              <label>
                بیمه
                <input [(ngModel)]="patientProfileForm.insuranceName" name="patientInsuranceName" placeholder="اختیاری" />
              </label>
              <label>
                توضیحات پرونده
                <input [(ngModel)]="patientProfileForm.notes" name="patientNotes" placeholder="اختیاری" />
              </label>
            </div>
          </section>
...
export class ConsultantDashboardComponent implements OnInit, OnDestroy {
  ...
  private validatePatientProfileForm(): string | null {
    ...
    if (!this.patientProfileForm.address.trim() || this.patientProfileForm.address.trim().length < 5) return 'آدرس بیمار الزامی است';
    const emergencyPhoneNumber = this.patientProfileForm.emergencyPhoneNumber.trim();
    if (emergencyPhoneNumber && !/^09\d{9}$/.test(emergencyPhoneNumber)) return 'شماره اضطراری بیمار معتبر نیست';
  ...
  private buildCompletePatientProfileRequest(reservationId: number): CompletePatientProfileRequest {
    return {
      reservationId,
