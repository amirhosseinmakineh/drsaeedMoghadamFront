    ...
    <section class="page-section contact-hero">
      <div class="section-heading">
        <h1>
          {{
            language() === "fa"
              ? "برای مشاوره انسانی، شماره خود را بگذارید یا مستقیم تماس بگیرید"
              : "For human guidance, leave your number or call directly"
          }}
        </h1>
        <p>
          {{
            language() === "fa"
              ...
              : "For guidance on composite veneers, porcelain veneers or dental bleaching, leave your number so the suitable visit path can be coordinated."
          }}
        </p>
      </div>
      <div class="contact-actions">
        <a class="primary-btn" href="#contact-request"
          ><app-fa-icon name="phone"></app-fa-icon
          >{{ language() === "fa" ? "ثبت درخواست تماس" : "Request a call" }}</a
        >
        <button class="secondary-btn" type="button" (click)="openAuth()">
    ...
    <section class="page-section contact-layout">
      <aside class="info-panel">
        ...
        <div class="map-card">
          ...
          <strong>{{
            language() === "fa" ? "هماهنگی مسیر مراجعه" : "Visit coordination"
          }}</strong>
          <span>{{
            language() === "fa"
              ? "پس از ثبت درخواست، اطلاعات لازم برای مراجعه و راهنمای مسیر توسط کلینیک اعلام می‌شود."
              : "After submitting a request, the clinic shares visit details and directions."
          }}</span>
        </div>
      </aside>
      ...
      <form id="contact-request" class="contact-form" (ngSubmit)="submit()">
        <h2>
          {{
            language() === "fa"
              ? "فرم درخواست تماس مشاور"
              : "Consultant call request form"
          }}
        </h2>
        <label>
        ...
        <app-base-datepicker
          ...
          (dateChange)="form.preferredDate = $event"
        ></app-base-datepicker>
        <label>
        ...
        <p *ngIf="sent()" class="success-message">
          {{
            language() === "fa"
              ? "درخواست شما ثبت شد. مشاور کلینیک برای راهنمایی با شما تماس می‌گیرد."
              : "Your request has been recorded. A clinic consultant will call you for guidance."
          }}
        </p>
      </form>
    </section>
