...
@Component({
  ...
  `,
  styles: [`
    .contact-hero{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:22px;align-items:end;padding-top:130px}.contact-hero h1{font-size:clamp(1.9rem,4vw,3.35rem);margin:0 0 16px}.contact-actions{display:flex;flex-wrap:wrap;gap:12px}.contact-layout{display:grid;grid-template-columns:minmax(290px,.8fr) minmax(0,1.2fr);gap:24px}.info-panel,.contact-form{display:grid;gap:14px}.info-panel article,.map-card,.contact-form{padding:22px;border:1px solid var(--line);border-radius:30px;background:color-mix(in srgb,var(--surface) 82%,transparent);box-shadow:var(--shadow);backdrop-filter:blur(18px)}.info-panel article{display:grid;grid-template-columns:auto 1fr;gap:14px}.info-panel h3,.info-panel p{margin:0}.map-card{display:grid;place-items:center;min-height:230px;text-align:center;color:var(--muted);background:radial-gradient(circle,color-mix(in srgb,var(--gold) 22%,transparent),transparent 62%),var(--surface)}.map-card app-fa-icon{font-size:2.6rem;color:var(--brand)}.map-card strong{color:var(--text)}.contact-form label{display:grid;gap:8px;color:var(--muted);font-weight:900}.success-message{margin:0;padding:12px 14px;border-radius:18px;background:color-mix(in srgb,#22c55e 14%,var(--surface));color:var(--text);font-weight:800}@media(max-width:860px){.contact-hero,.contact-layout{grid-template-columns:1fr}.contact-hero{padding-top:112px}}
  `]
})
