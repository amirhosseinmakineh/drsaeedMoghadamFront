...
export class PatientFinanceApiService {
  ...
  deletePromissoryNote(id: number) { return this.http.delete<ApiResult<IdResponse>>(`${this.baseUrl}/patient-promissory-notes/${id}`, this.options()); }
  ...
  getTransactions(query: PageQuery) { return this.get<PaginatedResult<PatientFinancialTransaction>>(`${this.baseUrl}/patient-financial-transactions`, query); }
  getDueCommitments(query: PageQuery) { return this.get<PaginatedResult<PatientFinancialCommitment>>(`${this.baseUrl}/patient-financial-commitments/due`, query); }

  private get<T>(url: string, query: PageQuery = {}): Observable<T> {
    return this.http.get<T | ApiResult<T>>(url, this.options(query)).pipe(
      map(response => {
        if (this.isApiResult(response)) {
          if (!response.isSuccess || response.data === null) {
            throw new Error(response.message || "دریافت اطلاعات مالی انجام نشد.");
          }
  ...
  private isApiResult<T>(response: T | ApiResult<T>): response is ApiResult<T> {
    return typeof response === "object" && response !== null && "isSuccess" in response && "data" in response;
  }
}
