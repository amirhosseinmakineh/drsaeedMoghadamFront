...
export class FollowUpService {
  ...
  private getPage<T>(url: string, query: Record<string, string | number>, fallback: string): Observable<PaginatedResponse<T>> { let params = new HttpParams(); Object.entries(query).forEach(([key, value]) => { if (value !== "") params = params.set(key, String(value)); }); return this.http.get<unknown>(url, { headers: this.headers(), params }).pipe(map(response => { const value = this.unwrap<Record<string, unknown>>(response); const items = Array.isArray(value.items) ? value.items as T[] : []; return { items, page: Number(value.page ?? query.page ?? 1), pageSize: Number(value.pageSize ?? query.pageSize ?? 20), totalCount: Number(value.totalCount ?? items.length) }; }), catchError(error => this.error(error, fallback))); }
